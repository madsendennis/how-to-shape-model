...
@main def ssmComponentExport() =
     val ssm = StatisticalModelIO.readStatisticalMeshModel(new File("data/vertebrae/pca.h5.json")).get
     val outputDir = new File("data/tmp/")
     outputDir.mkdirs()

     val valstop     = 25
     val stepSize    = 5
     val rangeInts   = (0 to valstop by stepSize) ++ (valstop to -valstop by -stepSize) ++ (-valstop to 0 by stepSize)
     ...
     def processComponent(comp: Int): Unit = {
          rangeDouble.foreach { v =>
               ...
               MeshIO.writeMesh(instance, new File(outputDir, s"${"ssm_%04d".format(cnt)}.ply"))
               cnt += 1
          }
     }
