...
@main def alignMeshes() =
  ...
  rawDataFolder.listFiles().filter(_.getName.endsWith(".ply")).foreach { f =>
    ...
    val transform = LandmarkRegistration
      .rigid3DLandmarkRegistration(landmarks, lms, Point3D(0, 0, 0))
    ...
    LandmarkIO.writeLandmarksJson[_3D](
      alignedLms,
      new File(alignedDataFolder, f.getName.replace(".ply", ".json"))
    )
  }
